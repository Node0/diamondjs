 ...
 *     It returns DATA the router executes — never a side effect.
 *
 * Fail-closed by construction: the base `check` returns false (an
 * unoverridden guard denies), and the base `deny` conceals (redirect to
...
export type Deny = { path: string } | { external: string }
...
export abstract class Guard {
  ...
  static deny(ctx: GuardContext): Deny {
    void ctx
